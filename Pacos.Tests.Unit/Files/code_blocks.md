Вот тебе примеры, как это выглядит в разных языках.
...
```
Можно даже настроить режим потокобезопасности, если уверен, что свойство будет использоваться только из одного потока (`LazyThreadSafetyMode.NONE`), чтобы избежать оверхеда на блокировки.

### Python
В Python нет встроенного ключевого слова, но ленивые свойства легко реализуются через дескрипторы или декораторы.

Вот пример с декоратором:
```python
class lazy_property:
